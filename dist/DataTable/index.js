...
function DataTable(_ref) {
  ...
  var controlledTableSelections = [selections, selectionsDispatch]; // Use the state and functions returned from useTable to build your UI

  var instance = useTable.apply(void 0, _toConsumableArray(tableArgs)); // Call ``fetchData`` whenever the state of the table changes (e.g., page change, sort or filter applied) but ignore
  // any state changes to current row selections as we don't want to re-fetch data whenever row(s) are selected.

  var tableStateWithoutSelections = _objectSpread({}, instance.state);
